# LOCATION: services/notification_service/notification_service/tenant_db.py

"""
Tenant DB connection resolution -- the same scoped-down pattern as
`tenant_service/tenant_db.py` and `job_service/tenant_db.py` (Section
2/Task E's full connection-routing middleware is still a future shared
piece). This service only ever needs READ access to ONE tenant DB per
event, to resolve which recruiters/company_admins to notify -- it never
opens a session with intent to write.
"""
