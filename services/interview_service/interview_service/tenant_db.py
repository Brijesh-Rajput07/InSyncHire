# LOCATION: services/interview_service/interview_service/tenant_db.py

"""
Tenant DB connection resolution -- same scoped-down pattern as
`tenant_service/tenant_db.py` / `job_service/tenant_db.py` (Section
2/Task E's full connection-routing middleware is still a future shared
piece; each service that needs one tenant DB per request builds this
same way using `shared.db` until enough services exist to justify
extracting it).
"""
